...
async def get_subtitle_config():
    """获取字幕配置，带兜底默认值"""
    try:
        config = await get_config_with_fallback('subtitle')
        # 确保所有默认配置项都存在
        if isinstance(config, dict):
            for key, default_value in DEFAULT_CONFIG['subtitle'].items():
                if key not in config:
                    config[key] = default_value
        else:
            config = DEFAULT_CONFIG['subtitle'].copy()
        return BaseResponse(data=config)
    except Exception as e:
        logger.error(f"Error getting subtitle config: {e}")
        # 返回兜底默认值
        return BaseResponse(data=DEFAULT_CONFIG['subtitle'].copy())
