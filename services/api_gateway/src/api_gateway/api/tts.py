...
async def get_tts_config():
    """获取TTS配置，带兜底默认值"""
    try:
        config = await get_config_with_fallback('tts')
        # 确保所有默认配置项都存在
        if isinstance(config, dict):
            for key, default_value in DEFAULT_CONFIG['tts'].items():
                if key not in config:
                    config[key] = default_value
        else:
            config = DEFAULT_CONFIG['tts'].copy()
        return BaseResponse(data=config)
    except Exception as e:
        logger.error(f"Error getting TTS config: {e}")
        # 返回兜底默认值
        return BaseResponse(data=DEFAULT_CONFIG['tts'].copy())
